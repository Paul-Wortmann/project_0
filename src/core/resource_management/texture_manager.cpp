...
bool texture_manager_class::load_texture(texture_type *texture)
{
    ...
    {
        return_value = false;
        game.core.log.write("Failed to load image ->"+texture->data.path);
    }
...
bool texture_manager_class::load_sprite_sheet(texture_type *texture, int width_set, int height_set)
{
    ...
    if ((sprite_sheet = game.core.file.load_image(texture->data.path.c_str())))
    {
        ...
        game.core.log.write("Loaded sprite sheet ->"+texture->data.path);
    }
    else
    {
        return_value = false;
    ...
    std::cout << texture->data.frame_max << std::endl;
...
{
    if (texture->data.text.font == NULL) texture->data.text.font = game.core.font_manager.root;
    bool return_value   = false;
    if ((texture->data.text.text_string.length() > 0) && (texture->data.text.font->font_data != NULL))
    {
        ...
        else
        {
            return_value = false;
            if (image_surface) SDL_FreeSurface(image_surface);
            game.core.log.write("Failed to load string ->"+texture->data.text.text_string);
        }
        if (image_surface) SDL_FreeSurface(image_surface);
        texture->data.loaded = return_value;
    }
